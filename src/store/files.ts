...
const useFileStore = create<FileState & FileAction>()((set, get) => ({
  currentId: initId,
  files: [
    {
      id: initId,
  ...
  updateBoardData(data) {
    const files = get().files
    const updateIndex = files?.findIndex((file) => file.id === get().currentId)
    set(
      produce((state) => {
  ...
  saveJSON() {
    const currentData = get().files?.find((file) => file.id === get().currentId)
    if (currentData) {
